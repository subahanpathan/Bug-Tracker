...
router.get('/', authMiddleware, asyncHandler(async (req, res) => {
  ...
  if (projectId) {
    ...
    if (project.owner_id !== req.userId && !member) {
      return res.status(403).json({ status: 'error', message: 'Forbidden - You do not have access to this project' });
    }

    query = query.eq('project_id', projectId);
  } else {
    // Fetch bugs for all projects user has access to (owned or member)
    ...
    const ownedProjectIds = ownedProjects?.map(op => op.id) || [];
    const allUserProjectIds = [...new Set([...memberProjectIds, ...ownedProjectIds])];

    if (allUserProjectIds.length === 0) {
      return res.status(200).json({ status: 'success', data: [] });
    }

    query = query.in('project_id', allUserProjectIds);
  }
